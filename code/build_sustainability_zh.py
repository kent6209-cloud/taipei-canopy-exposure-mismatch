...
OUT = ROOT / '論文_全文_v15_Sustainability中文版.docx'
OUTDIR = None
OUT_MAIN = ROOT / '論文_全文_v15_Sustainability中文版_主文.docx'
OUT_SUPP = ROOT / '論文_全文_v15_Sustainability中文版_附錄（補充材料）.docx'
...
F_CAPTION = re.compile(r'^(\**圖\s|\*Figure\s|圖\s*[A-Z0-9])')
REF_LIKE = re.compile(r'^\**\d{1,3}\.\s')


# --- Supplementary numbering (P0-7) -----------------------------------------
SUPP_MAP = [
    ('附錄圖 A-1', 'Figure S5-1'), ('附錄圖 A-2', 'Figure S7-1'), ('附錄圖 A-3', 'Figure S9-1'),
    ('附錄圖 A-4', 'Figure S10-1'), ('附錄圖 A-5', 'Figure S11-1'), ('附錄圖 A-6', 'Figure S8-1'),
...
SUPP_INDEX = [
    ...
    ('Supplementary S5', '不確定性傳播與 CHMv2 樹冠高度驗證（Uncertainty propagation; Figure S5-1）'),
    ('Supplementary S6', '圖版索引（Figure index and submission mapping）'),
    ('Supplementary S7', '全臺鄉鎮市區外部對照（National township comparison; Figure S7-1）'),
    ('Supplementary S8', '全臺 22 縣市對照與交叉檢核（County comparison; Figure S8-1）'),
    ('Supplementary S9', '官方山坡地與使用分區交叉檢核（Slope land and zoning; Figure S9-1）'),
    ('Supplementary S10', '年 NPP 通量產品可用性檢核（Flux-product usability audit; Figure S10-1）'),
    ('Supplementary S11', '行道樹結構分解與區級樹冠結構（Street-tree structure; Figure S11-1）'),
]
...
BACK_MATTER = [
    ...
    ('Data Availability Statement: All datasets used in this study are publicly available; the '
     ...
     'and analysis scripts are available from the corresponding author on reasonable request.'),
    ...
    for child in children:
        ...
        if sn == 'Heading 2' and text == '附錄':
            ...
            if mode == 'supp':
                ...
                add_para(doc, '本補充材料以 S1–S11 編號；主文之引註已同步對應（例：「附錄 E」於投稿版為 Supplementary S5）。',
